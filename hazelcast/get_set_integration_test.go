...
func TestGetSetIntegrationRoundTrip(t *testing.T) {
	...
	h, err := New(cfg, nil, 100*time.Millisecond)
	if err != nil {
		t.Fatalf("New: %v", err)
	}
	...
	if err := h.Set("k", want, 50*time.Millisecond); err != nil {
		t.Fatalf("Set: %v", err)
	}
	...
	// Wait beyond ttl + stale + safety margin and confirm expiration.
	time.Sleep(50*time.Millisecond + 100*time.Millisecond + 250*time.Millisecond)
